...
impl Sha256 {
    ...
    fn process_block(&mut self) {
        let mut w = [0u32; 64];
        for i in 0..16 {
            w[i] = u32::from_be_bytes(self.buffer[i * 4..(i + 1) * 4].try_into().unwrap());
        }
    ...
    pub fn finalize(mut self) -> [u8; 32] {
        let total_bits = (self.block_count * 512) + (self.buffer.len() as u64 * 8);
        ...
        self.buffer.extend_from_slice(&(total_bits as u64).to_be_bytes());
        self.process_block();
